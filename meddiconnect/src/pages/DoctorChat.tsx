 ...
 * Doctor Chat Page
 * 
 * Simple chat interface for doctors to communicate with their patients.
 * 
 * Features:
 * - View all chat channels (one per patient)
...
const DoctorChat: React.FC = () => {
  ...
  // Fetch all chat channels for the doctor
  useEffect(() => {
        ...
        {loading ? (
          ...
          <Paper sx={{ p: 4, textAlign: 'center' }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
              Chat channels will appear here once you have a confirmed appointment with a patient.
            </Typography>
          </Paper>
        ) : (
                      ...
                      >
                        <ListItemAvatar>
                          <Badge badgeContent={channel.unreadCount} color="primary">
                            <Avatar 
                              src={channel.patient.profilePicture}
                              sx={{ width: { xs: 40, md: 48 }, height: { xs: 40, md: 48 } }}
                            >
                              {channel.patient.firstName[0]}{channel.patient.lastName[0]}
                            </Avatar>
                          </Badge>
                        </ListItemAvatar>
                        <ListItemText
                          primary={
                            <Typography variant="body1" sx={{ fontSize: { xs: '0.875rem', md: '1rem' } }}>
                              {channel.patient.firstName} {channel.patient.lastName}
                            </Typography>
                          }
                          secondary={
                            <Typography 
              ...
              <Paper sx={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
                ...
                <Box sx={{ p: { xs: 1.5, md: 2 }, borderBottom: 1, borderColor: 'divider' }}>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 1, md: 2 } }}>
                    ...
                    <Avatar 
                      src={selectedChannel.patient.profilePicture}
                      sx={{ width: { xs: 40, md: 48 }, height: { xs: 40, md: 48 } }}
                    >
                      {selectedChannel.patient.firstName[0]}{selectedChannel.patient.lastName[0]}
                    </Avatar>
                    <Box sx={{ flex: 1, minWidth: 0 }}>
                      ...
                      >
                        {selectedChannel.patient.firstName} {selectedChannel.patient.lastName}
                      </Typography>
                      <Typography 
                        variant="body2" 
                ...
                >
                  {messages.map((message) => {
                    // Compare senderId with channel's doctor ID (since this is DoctorChat, the current user is the doctor)
                    const isOwnMessage = selectedChannel && message.senderId === selectedChannel.doctor.id;
                    ...
                    // For own messages, get user info from selected channel (doctor info)
                    const ownUserInfo = isOwnMessage && selectedChannel 
                      ? selectedChannel.doctor 
                      : null;
